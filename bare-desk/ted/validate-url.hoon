...
::
++  url-path  '/.well-known/appspecific/org.urbit.beacon.json'
::
++  turf-to-url
  |=  =turf
  ^-  @t
...
?:  (gte retry 3)
  ...
  [leaf+"Sentinel URL validation thread log:" (flop tang)]
?:  (gte redirect 5)
  %=  loop
    retry     +(retry)
    redirect  0
...
::
  ...
  ?~  bod.u.res
    ...
    [leaf+"Sentinel thread log:" (flop tang)]
  ...
  ?~  jon
    ...
    %+  strand-fail:strandio  %parsing-failed
    [leaf+"Sentinel thread log:" (flop tang)]
  =/  man=(unit manifest:beacon)  (manifest-soft:dejs:sentinel u.jon)
  ?~  man
  ...
  =/  result=status
    (process-manifest:sentinel turf ship lyfe keys u.man)
  (pure:m !>(result))
::
